...
export function useFolderContents(folderId: string | null, categoryFilter?: string, searchTerm?: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<FolderContents> => {
      ...
      if (categoryFilter && categoryFilter !== "Todas") {
        // For each folder, check if it contains documents with the selected category
        // This requires checking documents in each folder (including subfolders)
        const folderIds = filteredFolders.map((f) => f.id);
        console.log(`[Folder Contents] Filtering ${folderIds.length} folders by category: "${categoryFilter}"`);
        
        if (folderIds.length > 0) {
          try {
            const categoryFilterNormalized = normalizeCategoryName(categoryFilter);
            console.log(`[Folder Contents] Category filter normalized: "${categoryFilter}" -> "${categoryFilterNormalized}"`);
            
            // Check each folder individually to see if it or its subfolders contain matching documents
            const folderMatches = await Promise.all(
              filteredFolders.map(async (folder) => {
                // Get all subfolder IDs for this specific folder (including the folder itself)
                const allSubfolderIds = await getAllSubfolderIds([folder.id]);
                console.log(`[Folder Contents] Folder "${folder.name}" has ${allSubfolderIds.length} total folders in hierarchy`);
                
                // Search for documents in this folder's hierarchy
                const { data: folderDocuments, error: folderDocsError } = await supabase
                  .from("documents")
                  .select("parent_folder_id, category")
                  .in("parent_folder_id", allSubfolderIds)
                  .eq("is_published", true);
                ...
                if (folderDocsError) {
                  console.error(`[Folder Contents] Error fetching documents for folder "${folder.name}":`, folderDocsError);
                  return false;
                }
                
                if (!folderDocuments || folderDocuments.length === 0) {
                  console.log(`[Folder Contents] Folder "${folder.name}" has no documents`);
                  return false;
                }
                
                // Check if any document matches the category
                const hasMatchingDoc = folderDocuments.some((doc) => {
                  const docCategoryNormalized = normalizeCategoryName(doc.category || "");
                  const matches = docCategoryNormalized === categoryFilterNormalized;
                  if (matches) {
                    console.log(`[Folder Contents] Folder "${folder.name}" has matching document: "${doc.category}" in folder ${doc.parent_folder_id}`);
                  }
                  return matches;
                });
            ...
            filteredFolders = filteredFolders.filter((folder, index) => {
              const hasMatch = folderMatches[index];
              console.log(`[Folder Contents] Folder "${folder.name}" (${folder.id}): ${hasMatch ? 'INCLUDED' : 'EXCLUDED'}`);
              return hasMatch;
            });
              
            console.log(`[Folder Contents] Filtered folders: ${filteredFolders.length} out of ${folders.length} folders`);
          } catch (error) {
            console.error("[Folder Contents] Error filtering folders by category:", error);
            // Em caso de erro, retornar lista vazia (não mostrar todas as pastas)
            filteredFolders = [];
          }
        }
      }
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<FolderContents> => {
      ...
      if (categoryFilter && categoryFilter !== "Todas") {
        // For each folder, check if it contains documents with the selected category
        const folderIds = filteredFolders.map((f) => f.id);
        console.log(`[Root Contents] Filtering ${folderIds.length} folders by category: "${categoryFilter}"`);
        
        if (folderIds.length > 0) {
          try {
            const categoryFilterNormalized = normalizeCategoryName(categoryFilter);
            console.log(`[Root Contents] Category filter normalized: "${categoryFilter}" -> "${categoryFilterNormalized}"`);
            ...
            const folderMatches = await Promise.all(
              filteredFolders.map(async (folder) => {
                // Get all subfolder IDs for this specific folder (including the folder itself)
                const allSubfolderIds = await getAllSubfolderIds([folder.id]);
                console.log(`[Root Contents] Folder "${folder.name}" has ${allSubfolderIds.length} total folders in hierarchy`);
                ...
                if (folderDocsError) {
                  console.error(`[Root Contents] Error fetching documents for folder "${folder.name}":`, folderDocsError);
                  return false;
                }
                
                if (!folderDocuments || folderDocuments.length === 0) {
                  console.log(`[Root Contents] Folder "${folder.name}" has no documents`);
                  return false;
                }
                
                // Check if any document matches the category
                const hasMatchingDoc = folderDocuments.some((doc: any) => {
                  const docCategoryNormalized = normalizeCategoryName(doc.category || "");
                  const matches = docCategoryNormalized === categoryFilterNormalized;
                  if (matches) {
                    console.log(`[Root Contents] Folder "${folder.name}" has matching document: "${doc.category}" in folder ${doc.parent_folder_id}`);
                  }
                  return matches;
                });
            ...
            filteredFolders = filteredFolders.filter((folder, index) => {
              const hasMatch = folderMatches[index];
              console.log(`[Root Contents] Folder "${folder.name}" (${folder.id}): ${hasMatch ? 'INCLUDED' : 'EXCLUDED'}`);
              return hasMatch;
            });
              
            console.log(`[Root Contents] Filtered folders: ${filteredFolders.length} out of ${folders.length} folders`);
          } catch (error) {
