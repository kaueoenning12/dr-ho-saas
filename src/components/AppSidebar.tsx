...
export function AppSidebar() {
  ...
  // Memoizar menuItems para evitar recriação a cada render
  const menuItems = useMemo(() => {
    const items = [
      { title: "Home", url: "/", icon: Home },
      { title: "Relatórios", url: "/documents", icon: FileText },
        ...
        <SidebarGroup className={isCollapsed ? 'p-0' : ''}>
          ...
          <SidebarGroupContent>
            <SidebarMenu className={`space-y-1 ${isCollapsed ? 'mx-auto' : ''}`}>
              {menuItems.map((item) => {
                // Verificar se a rota está ativa manualmente
                  ...
                  >
                    <SidebarMenuButton asChild>
                      <NavLink
                        ...
                        className={`group/item flex ${isCollapsed ? '!w-auto mx-auto' : 'w-full'} items-center ${isCollapsed ? 'justify-center' : 'gap-3'} ${isCollapsed ? 'px-2' : 'px-3'} py-2.5 rounded-xl font-medium text-[14px] transition-all duration-300 relative ${isCollapsed ? '' : 'overflow-hidden'} ${
                          isActive
                            ? "bg-cyan/20 text-cyan shadow-sm border-l-2 border-cyan font-semibold"
                            : `text-sidebar-foreground/80 hover:text-sidebar-foreground hover:bg-foreground/10 ${isCollapsed ? '' : 'hover:translate-x-1'}`
                        }`}
                        onClick={(e) => {
                          // Prevenir recarregamento se já estiver na mesma rota
