...
import { useHomeAnnouncements, useDeleteHomeAnnouncement } from "@/hooks/useHomeAnnouncementsQuery";
import { useEvents, useDeleteEvent } from "@/hooks/useEventsQuery";
import { useNews, useDeleteNews, News } from "@/hooks/useNewsQuery";
...
export default function Admin() {
  ...
  const { data: homeAnnouncements = [], isLoading: homeAnnouncementsLoading } = useHomeAnnouncements(user?.role === "admin");
  const { data: events = [], isLoading: eventsLoading } = useEvents(user?.role === "admin");
  const { data: news = [], isLoading: newsLoading } = useNews(user?.role === "admin");
  const deleteHomeAnnouncement = useDeleteHomeAnnouncement();
  ...
  const stats = [
    { label: "Total de Relatórios", value: totalDocuments, icon: FileText },
              ...
              <TabsContent value="documents" className="mt-0">
                <Card className="border border-cyan/10 shadow-elegant">
                  <CardHeader className="px-4 sm:px-6 py-4 sm:py-5 border-b border-cyan/10">
                    <CardTitle className="text-[16px] sm:text-[18px] font-semibold text-navy">Relatórios Publicados</CardTitle>
                    <CardDescription className="text-[13px] sm:text-[14px] font-light mt-1 text-navy/60">
                      Gerencie os relatórios disponíveis na plataforma
                    </CardDescription>
                  </CardHeader>
                        ...
                        {documents.map((doc) => (
                          ...
                          >
                            <div className="flex-1 min-w-0 mr-3 sm:mr-4">
                              <p className="font-medium text-[14px] sm:text-[15px] text-foreground truncate">
                                {doc.title}
                              </p>
                              <p className="text-[12px] sm:text-[13px] text-muted-foreground font-light mt-0.5">
                                {doc.category}
                              </p>
                            </div>
                            <div className="flex items-center gap-3 sm:gap-4 shrink-0">
                              <Badge
                                variant="outline"
                                className="bg-cyan/10 text-cyan border-cyan/30 text-[12px]"
                              >
                  ...
                  <CardHeader className="px-4 sm:px-6 py-4 border-b border-cyan/10">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div>
                        ...
                        <CardDescription className="text-[13px] sm:text-[14px] font-light mt-1 text-navy/60">
                          Visualize todas as avaliações feitas pelos usuários nos relatórios premium
                        </CardDescription>
                      </div>
                      <div className="relative w-full sm:w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                          ...
                          <TableBody>
                            {filteredRatings.map((rating) => (
                              <TableRow key={rating.id} className="hover:bg-cyan/5">
                                ...
                                <TableCell>
                                  <div>
                                    <p className="font-medium text-sm text-navy">{rating.document?.title || "Relatório"}</p>
                    ...
                    <CardHeader className="px-4 sm:px-6 py-4 sm:py-5 border-b border-cyan/10">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription className="text-[13px] sm:text-[14px] font-light mt-1 text-navy/60">
                            Gerencie os avisos exibidos na tela inicial
                          </CardDescription>
                        </div>
                        <HomeAnnouncementDialog onSuccess={() => {}} />
                      </div>
                    </CardHeader>
                    <CardContent className="p-4 sm:p-6">
                      {homeAnnouncementsLoading ? (
                        <div className="text-center py-10">
                          <div className="inline-block h-6 w-6 animate-spin rounded-full border-3 border-solid border-cyan border-r-transparent"></div>
                          <p className="text-muted-foreground text-sm mt-2">Carregando...</p>
                        </div>
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          {homeAnnouncements.map((announcement: HomeAnnouncement) => (
                            <div
                              key={announcement.id}
                              className="flex items-center justify-between p-3 sm:p-4 border border-cyan/10 rounded-lg hover:bg-cyan/5 hover:border-cyan/30 transition-all duration-200"
                            >
                              <div className="flex-1 min-w-0 mr-3 sm:mr-4">
                                ...
                                <p className="text-[12px] sm:text-[13px] text-muted-foreground font-light mt-0.5">
                                  Prioridade: {announcement.priority === "high" ? "Alta" : announcement.priority === "medium" ? "Média" : "Baixa"} | Ordem: {announcement.order}
                                </p>
                              </div>
                              <div className="flex items-center gap-2 shrink-0">
                                ...
                                <HomeAnnouncementDialog announcement={announcement} onSuccess={() => {}} />
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => {
                                    if (confirm("Tem certeza que deseja excluir este aviso?")) {
                                      deleteHomeAnnouncement.mutate(announcement.id);
                                    }
                                  }}
                    ...
                    <CardHeader className="px-4 sm:px-6 py-4 sm:py-5 border-b border-cyan/10">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription className="text-[13px] sm:text-[14px] font-light mt-1 text-navy/60">
                            Gerencie os eventos exibidos na tela inicial
                          </CardDescription>
                        </div>
                        <EventDialog onSuccess={() => {}} />
                      </div>
                    </CardHeader>
                    <CardContent className="p-4 sm:p-6">
                      {eventsLoading ? (
                          ...
                          {events.map((event: Event) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                ...
                                <EventDialog event={event} onSuccess={() => {}} />
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => {
                    ...
                    <CardHeader className="px-4 sm:px-6 py-4 sm:py-5 border-b border-cyan/10">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription className="text-[13px] sm:text-[14px] font-light mt-1 text-navy/60">
                            Gerencie as novidades exibidas na tela inicial
                          </CardDescription>
                        </div>
                        <NewsDialog onSuccess={() => {}} />
                      </div>
                    </CardHeader>
                    <CardContent className="p-4 sm:p-6">
                      {newsLoading ? (
                          ...
                          {news.map((item: News) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                ...
                                <NewsDialog news={item} onSuccess={() => {}} />
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => {
