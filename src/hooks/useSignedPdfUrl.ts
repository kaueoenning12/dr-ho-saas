...
// Cache for signed URLs with TTL (1 hour expiration)
...
const CACHE_EXPIRY_MS = 55 * 60 * 1000; // 55 minutes (slightly less than 1 hour to be safe)
  ...
  useEffect(() => {
    ...
    const generateSignedUrl = async () => {
      try {
        ...
        // Check cache first
        const cached = signedUrlCache.get(pdfUrl);
        if (cached && cached.expiresAt > Date.now()) {
          console.log("[useSignedPdfUrl] Using cached signed URL");
        ...
        if (pdfUrl.startsWith('http://') || pdfUrl.startsWith('https://')) {
          // Se já é uma URL assinada, usar diretamente e cachear
          if (pdfUrl.includes('/object/sign/')) {
            console.log("[useSignedPdfUrl] URL já é assinada, usando diretamente");
            // Cache the already signed URL
            signedUrlCache.set(pdfUrl, {
              url: pdfUrl,
          ...
          if (pdfUrl.includes('/object/public/')) {
            ...
            // URL completa mas não reconhecida, tentar usar diretamente e cachear
