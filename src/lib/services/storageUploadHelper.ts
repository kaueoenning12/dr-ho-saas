 ...
 * FASE 4: Melhor tratamento de erros e logging
 */
export async function uploadWithFetchDirect(
  ...
  if (!session?.access_token) {
    console.log('[Storage Helper] Sem token, tentando refresh automático...');
    const { data: { session: refreshedSession }, error: refreshError } = await supabase.auth.refreshSession();
    
    if (refreshedSession?.access_token) {
      session = refreshedSession;
      console.log('[Storage Helper] ✅ Token obtido via refresh');
    } else {
      console.error('[Storage Helper] ❌ Não foi possível obter token:', refreshError?.message);
      throw new Error('Sessão expirada. Por favor, faça login novamente.');
    }
  }

  if (sessionError) {
    console.error('[Storage Helper] Erro ao obter sessão:', sessionError);
  ...
  while (retries > 0) {
    try {
      ...
      // Se for erro de acesso negado, dar mensagem clara
      if (response.status === 403) {
        throw new Error(`Acesso negado (403). Verifique se você tem role de admin ou moderador. Detalhes: ${errorText}`);
      }
      
      // Se for erro 401, sessão expirou
