...
function DocumentViewContent() {
  ...
  // Lazy load related documents - only fetch when document is loaded and category is available
  // For now, we'll load them but with a longer staleTime to reduce refetches
  const { data: relatedDocuments = [] } = useDocuments({
    category: document?.category,
  }, {
    enabled: !!document?.category && document.category !== "Todas",
    staleTime: 10 * 60 * 1000, // Cache for 10 minutes
  });

  // Filter out current document and limit to 5
  const relatedDocs = useMemo(() => {
    if (!relatedDocuments || relatedDocuments.length === 0) return [];
            ...
            <div className="px-3 sm:px-4 md:px-6 pb-3 border-t border-cyan/10">
              {isDocumentLoading ? (
                ...
                <div className="flex items-center justify-between">
                  <h1 className="text-lg font-semibold text-foreground">Relatório não encontrado</h1>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => navigate("/documents")}
                  ...
                  {signedUrlError ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-sm font-medium text-muted-foreground">
                          Gerando acesso seguro ao relatório...
                        </p>
                      </div>
                    </div>
                  )}
