...
import { uploadWithFetchDirect, uploadWithEdgeFunction } from "./storageUploadHelper";
import { generateFolderIcon } from "./openaiIconService";
...
export async function createFolderInDatabase(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // Generate and cache icon asynchronously (don't block folder creation)
      generateFolderIcon({ folderName })
        .then((svg) => {
          if (svg) {
            try {
              localStorage.setItem(`folder-icon-${data.id}`, svg);
              console.log(`[Folder Creation] ✅ Icon generated and cached for folder: ${folderName}`);
            } catch (error) {
              console.warn(`[Folder Creation] ⚠️ Failed to cache icon:`, error);
            }
          }
        })
        .catch((error) => {
          console.warn(`[Folder Creation] ⚠️ Failed to generate icon for folder ${folderName}:`, error);
        });
...
export async function uploadFileToStorage(
  ...
  // Tentar upload direto com fetch primeiro, depois Edge Function como fallback
  try {
    ...
    console.log('[Folder Upload] ✅ Upload bem-sucedido via fetch direto!');
    return {
      publicUrl: result.publicUrl,
      filePath: storagePath,
    };
  } catch (fetchError: any) {
    console.warn('[Folder Upload] ⚠️ Fetch direto falhou, tentando Edge Function...', fetchError.message);
    
    // Tentar Edge Function como fallback
    try {
      const result = await uploadWithEdgeFunction(file, storagePath);
      console.log('[Folder Upload] ✅ Upload bem-sucedido via Edge Function!');
      return {
        publicUrl: result.publicUrl,
        filePath: storagePath,
      };
    } catch (edgeError: any) {
      console.error('[Folder Upload] ❌ Ambos métodos falharam:', { fetchError: fetchError.message, edgeError: edgeError.message });
      
      // Interpretar erro e dar mensagem útil
      const errorMsg = fetchError.message || edgeError.message || '';
      if (errorMsg.includes('403') || errorMsg.includes('denied') || errorMsg.includes('RLS')) {
        throw new Error(`Arquivo ${file.name}: Acesso negado. Verifique se você tem permissão de admin/moderador.`);
      } else if (errorMsg.includes('404') || errorMsg.includes('not found') || errorMsg.includes('bucket')) {
        throw new Error(`Arquivo ${file.name}: Bucket não encontrado. Verifique a configuração do Storage.`);
      } else if (errorMsg.includes('Token') || errorMsg.includes('autenticação') || errorMsg.includes('session')) {
        throw new Error(`Arquivo ${file.name}: Sessão expirada. Faça login novamente.`);
      } else {
        throw new Error(`Arquivo ${file.name}: ${errorMsg}`);
      }
    }
  }
}
  ...
  try {
    ...
    for (const fileInfo of filesToUpload) {
      try {
        ...
        // Primeiro tenta upload direto via fetch (mais simples/rápido)
        try {
          console.log('[Folder Upload] Tentando upload com fetch direto (pasta)...', {
            fileName: fileInfo.file.name,
            simplifiedPath,
          });
          const directResult = await uploadWithFetchDirect(fileInfo.file, simplifiedPath, 'documents');
          publicUrl = directResult.publicUrl;
          filePath = directResult.filePath;
          console.log('[Folder Upload] ✅ Upload bem-sucedido com fetch direto (pasta)');
        } catch (error: any) {
          const message: string = error?.message || String(error);
          console.error('[Folder Upload] ❌ Erro no upload com fetch direto (pasta):', error);

          // Se for erro de rede/CORS, tentar fallback via Edge Function
          ...
          if (isNetworkLikeError) {
            try {
              console.warn('[Folder Upload] Tentando fallback via Edge Function para upload de pasta...', {
                fileName: fileInfo.file.name,
                simplifiedPath,
              });
              const edgeResult = await uploadWithEdgeFunction(fileInfo.file, simplifiedPath);
              publicUrl = edgeResult.publicUrl;
              filePath = edgeResult.filePath;
              console.log('[Folder Upload] ✅ Upload bem-sucedido via Edge Function (pasta)');
            } catch (edgeError: any) {
              console.error('[Folder Upload] ❌ Erro também na Edge Function (pasta):', edgeError);
              throw edgeError;
            }
          } else {
            // Erro não relacionado a rede/CORS: propagar para o handler atual
            throw error;
          }
        }

        if (!filePath) {
          throw new Error(`Falha ao determinar caminho do arquivo após tentativas de upload para ${fileInfo.fileName}`);
        }
