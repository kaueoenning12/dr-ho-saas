...
import { Upload, FileText, Loader2, Folder, Plus, Trash2 } from "lucide-react";
...
import { useDocumentCategories, useCreateDocumentCategory, useDeleteDocumentCategory } from "@/hooks/useDocumentCategories";
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (uploadType === "file" && selectedFile) {
        ...
        toast({
          title: "Sucesso!",
          description: `Relatório "${data.title}" foi ${data.isPublished ? "publicado" : "salvo como rascunho"}.`,
        });
      } else if (uploadType === "folder" && selectedFiles) {
        // FASE 3: Refresh de token antes do upload de pasta
        console.log('[Folder Upload] Forçando refresh de token antes do upload...');
        const { data: { session: refreshedSession }, error: refreshError } = await supabase.auth.refreshSession();
        
        if (refreshError) {
          console.warn('[Folder Upload] Aviso no refresh de token:', refreshError.message);
        } else if (refreshedSession) {
          console.log('[Folder Upload] ✅ Token atualizado com sucesso!');
        }
        ...
        if (result.errors.length > 0) {
          ...
          toast({
            ...
            description: `${result.documentsCreated} relatório(s) criado(s). ${result.errors.length} erro(s) encontrado(s). ${errorDetails}${moreErrors}`,
            variant: "default",
          ...
          toast({
            title: "Sucesso!",
            description: `${result.documentsCreated} relatório(s) criado(s) em ${result.foldersCreated.length} pasta(s).`,
          });
        }
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto border-0 shadow-elegant-lg">
        <DialogHeader className="pb-4">
          <DialogTitle className="text-[20px] font-semibold tracking-tight text-navy">Enviar Novo Relatório</DialogTitle>
          <DialogDescription className="text-[14px] font-light mt-1 text-navy/60">
            {uploadType === "file" 
              ? "Adicione um novo relatório de segurança do trabalho à biblioteca. Qualquer tipo de arquivo é permitido."
              : "Faça upload de uma pasta inteira com relatórios. A estrutura de pastas será preservada. Qualquer tipo de arquivo é permitido."}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                            ...
                            {user?.role === "admin" && (
                              <span
                                ...
                                onPointerDown={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  if (!category.id || deleteCategory.isPending) return;
                                  deleteCategory.mutate(category.id, {
                                    onSuccess: () => {
                                      // Se a categoria deletada estava selecionada, limpar o campo
                                      if (form.getValues("category") === (category.name || "")) {
                ...
                {user?.role === "admin" && showNewCategoryInput && (
              ...
              render={({ field }) => (
                <FormItem className="flex items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>
                      Tornar este relatório visível para todos os usuários
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Imagem que será exibida borrada quando o relatório estiver bloqueado
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
